"""
node_timeout_high.py  --  Decentralized FL Node  (Experiment 5A-T90/120)
                         Timeout variant: push=90s, receive=120s
...
Usage:
    python3 node_timeout_high.py <node_id> <my_ip> <left_ip> <right_ip>
                   [--dist iid|non_iid] [--alpha F] [--rounds N] [--fault-demo]

Examples:
    python3 node_timeout_high.py 0 172.31.21.108 172.31.18.64 172.31.31.28 --fault-demo
"""
...
def run_node(node_id, my_ip, left_ip, right_ip,
    ...
    print()
    print(SEP)
    print("  DECENTRALIZED FL  --  NODE  [TIMEOUT VARIANT: push=90s receive=120s (Exp 5A-T90/120)]")
    print(SEP)
    print()
    ...
    print(f"  Timeout      : push=90s  receive=120s  (baseline values)")
    ...
    for round_num in range(1, num_rounds + 1):
        ...
        if fault_demo and node_id == 3 and round_num == FAIL_ROUND:
            print()
            print(SEP)
            print("  EXPERIMENT 5A-T90/120  --  NODE 3 FAILURE")
            print(SEP)
            print()
        ...
        if push_fail and fault_demo:
            log(f"      Push failure expected in Exp 5A-T90/120  --  neighbor node is down.")

        # Wait for receive to complete
        recv_t.join(timeout=125)
    ...
    print()
    print(SEP)
    print(f"  DECENTRALIZED FL  --  NODE {node_id}  --  RESULTS  [Exp 5A-T90/120]")
    print(SEP)
    print()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Decentralized FL Node  (Experiment 5A-T90/120  --  push=90s receive=120s)',
    ...
    parser.add_argument('--fault-demo', action='store_true',
                        help='Experiment 5A-T90/120: Node 3 exits at round 10 to demonstrate no-SPOF')

    args = parser.parse_args()

    if args.fault_demo:
        exp_label = 'decentralized_fault_t90_120'
